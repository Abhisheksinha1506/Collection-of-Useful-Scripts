...
If ( ($IncludeMinutia -eq "y") -or
     ($IncludeMinutia -eq "yes") ) {
     #ThenAccountForMinutiaSizeOpen
...
While ( ($IncludeSystemBackup -ne "y") -and
     ...
     ($IncludeSystemBackup -ne "no") ) {
     #IncludeSystemBackupOpen
     ...
     If (($VMwareRunning -ne $Null) -and (($IncludeSystemBackup -eq "yes") -or ($IncludeSystemBackup -eq "y")) ) {
          #VMsMightBeRunning
...
If ( ($IncludeSystemBackup -eq "y") -or
     ($IncludeSystemBackup -eq "yes") ) {
     #ThenIncludeWBOpen
     $DataSize = $DataSize + $SystemBackupAllocation 
} #ThenIncludeWBClose
...
ForEach ($DriveLetter in $TotalDrives) {
     #CountDrivesOpen
     If ($DriveLetter.DriveLetter -ne $Null) { $DriveCount += 1 } 
} #CountDrivesClose

While ($NumDrives -lt 0 -or $NumDrives -gt $DriveCount) {
     #WhileDrivesInvalidOpen
     $NumDrives = Read-Host "How many data drives to back up? " 
     If ($NumDrives -gt $DriveCount) {
          #TooManyDrivesOpen
...
$Drives = @()
For ($i = 1; $i -le $NumDrives; $i++) {
     #DriveLetterForLoopOpen
     $DriveLetter = 'X'
     $DrivePath = $DriveLetter + ":\"
     While ((-not (Test-Path $DrivePath) -or $DriveLetter -eq 'C')) {
          #WhileDriveLetterLoopOpen
          $DriveLetter = Read-Host "Enter a drive letter to backup. " 
          $DrivePath = $DriveLetter + ":\" 
     } #WhileDriveLetterLoopClose
     $Drives = $Drives + $DrivePath
} #DriveLetterForLoopClose

$BDrive = 'X'
$BPath = $BDrive + ":\"
While ((-not (Test-Path $Bpath) -or $BDrive -eq 'C')) {
     #WhileBackupDriveInvalidOpen
     #$BDrive = 'X'
     #$BPath = $BDrive +":\"
     $BDrive = Read-Host "Backup to what drive letter? "
     $BPath = $BDrive + ":\" 
} #WhileBackupDriveInvalidClose
...
ForEach ($Drive in $Drives) {
     ...
     $DataSize = $DataSize + [math]::Round($DriveSize, 2) 
} #AddUpDataToBeBackedUpClose

If ( ($IncludeMinutia -eq 'y') -or
     ($IncludeMinutia -eq 'yes') ) {
     #MinutiaIncludedOpen
     forEach ($Item in $Minutia) {
          #MinutiaSizeOpen
...
If ($DataSize -gt $BackupCapacity) {
     #NotEnoughBackupSpaceOpen
     Write-Host "Not enough room to backup $DataSize GB onto $BackupCapacity GB."
     Write-Host "Exiting."
     exit 
} #NotEnoughBackupSpaceClose
...
ForEach ($Drive in $Drives) {
     ...
     robocopy $SPath $DestPath /e /np /b /copyall /r:10 /w:0 /eta 
} #EachDriveLoopClose
